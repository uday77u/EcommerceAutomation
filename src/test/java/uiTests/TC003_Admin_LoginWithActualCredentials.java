...
public class TC003_Admin_LoginWithActualCredentials extends BaseUITest{

	@Test
	public void testAdmin_LoginWithActualCredentials() {
		try {
		...
		//logger.info(null);
		logger.info("start logging ***testAdmin_LoginWithActualCredentials***");
		...
		signInPage.enterEmailTxtFld(AdminEmail);
		
		logger.info("Step 5: Enter Password field");
		signInPage.enterPasswordTxtFld(AdminPassword);
        ...
        logger.error("❌ Assertion failed in testAdmin_LoginWithActualCredentials: " + ae.getMessage(), ae);
        throw ae; // rethrow so TestNG marks test as failed
    } catch (Exception e) {
        logger.error("❌ Unexpected exception in testAdmin_LoginWithActualCredentials: " + e.getMessage(), e);
