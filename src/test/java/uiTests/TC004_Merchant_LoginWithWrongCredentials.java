...
public class TC004_Merchant_LoginWithWrongCredentials extends BaseUITest{
	
	@Test
	public void testMerchant_LoginWithWrongCredentials() {
		try {
		...
		//logger.info(null);
		logger.info("start logging ***testMerchant_LoginWithWrongCredentials***");
		...
		logger.info("Step 3: Clicking on the 'Merchant Login'");
		signInPage.clickMerchantLoginBtn();
		...
		logger.info("Step 7: Verify the error message");
        ...
        logger.error("❌ Assertion failed in testMerchant_LoginWithWrongCredentials: " + ae.getMessage(), ae);
        throw ae; // rethrow so TestNG marks test as failed
    } catch (Exception e) {
        logger.error("❌ Unexpected exception in testMerchant_LoginWithWrongCredentials: " + e.getMessage(), e);
